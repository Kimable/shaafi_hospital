        ...
        <div class="row">
          ...
          <div class="col-lg-8">
            ...
            <form action="{{ route('appointment.post') }}" method="post">
              ...
              <div class="mb-3">
                <label for="doctor_email" class="form-label">{{ __('Preffered Doctor') }}</label>

                <select class="form-select" name="doctor_id" id="doctor_email">
                  <option value="Unspecified">--{{ __('Choose Doctor') }}--</option>
              ...
              <div class="mb-3">
                <label class="form-label" for="inquiry">{{ __('Medical Concern') }}</label>
                <input type="text" class="form-control" name="inquiry" id="inquiry" required>
              </div>

              <div class="mb-3">
                <label for="description" class="form-label">{{ __('Describe Condition') }}</label>
                <textarea class="form-control" name="description" id="description" style="height: 100px" required></textarea>
              </div>

              <button class="btn btn-primary w-100 py-2 my-3" type="submit">{{ __('Book Appointment') }}</button>
            </form>
          </div>
        </div>
      </div>
